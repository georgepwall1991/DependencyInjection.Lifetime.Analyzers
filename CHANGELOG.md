...
- **DI038: Container-owned service disposed by consumer** (Warning). The disposal-ownership family (DI008, DI014, DI032, DI033) covered every way an owner fails to dispose; this closes the inverse: a consumer disposing what the container owns while other consumers still need it. The injected shape reports a container-constructed consumer whose lifetime is strictly shorter than the dependency's (transient/scoped consumer of a singleton, transient consumer of a scoped service) disposing that constructor-injected dependency — directly, through `?.`, behind an `IDisposable`/`IAsyncDisposable` cast, on the parameter itself, or on a this-rooted private/readonly field or auto-property provably assigned only from never-rebound constructor parameters (primary-constructor field initializers included). Equal-lifetime pairs are container co-teardown double-disposes and stay silent, per the DI026 same-scope precedent. The resolved shape reports `using` over, or explicit disposal of, the exact framework `GetService<T>`/`GetRequiredService<T>` result for a singleton-registered service on a shared provider, including through an unreassigned local; a provider built by the resolving member itself is a throwaway container and stays silent. Only methods actually implementing the disposal interfaces count. Ownership proofs run over effective registrations (`Clear`/`RemoveAll`/`Replace` replayed) with slot-winner semantics for the consumer, keep transients DI008's finding, and stay silent for mixed lifetimes, keyed slots, pre-built instances (disposing one deliberately is DI033's remediation), factory-built or unregistered consumers, externally assignable members (public/settable/init), reassigned or `ref`-aliased or deconstructed parameters/locals/fields, null-conditional member writes, `if (owns)`-guarded dual-use disposal (the branch must imply the flag through `&&`-only structure; the flag must be constructor-written-only and externally unassignable; negated flags, disjunctions, ref-written or settable flags, and else-branches still report), pattern-only `DisposeAsync` resolutions the container never calls (while a disposable implementation behind a non-disposable abstraction still reports), provider/scope infrastructure types (DI001/DI014), hand-rolled provider classes, and user-defined resolution helpers. `AddMemoryCache`/`AddLogging` count as container-created and prove framework ownership; `AddHttpContextAccessor`/`AddHttpClient` deliberately do not (the container disposes neither). Cross-reviewed adversarially over six rounds (27 findings — one declined as a documented boundary, all fixed or accepted-and-documented, each fix with a mutation-tested regression test). Accepted false negatives are documented in the rule's guardrails. No code fix: the repair is deleting the call or restructuring ownership, which only the author can choose.
...
- DI020 now expands stable local reference-type arrays passed through `UseMiddleware<T>(params object[] args)`, preserving constructor binding for explicit middleware arguments and reporting remaining scoped dependencies.
...
- **DI002 caller-owned collection parameters** — scoped services now report when passed directly or through a tracked local to a mutating collection method on a parameter such as `destination.Add(service)`. The existing semantic guards still require a real enumerable receiver and a void/bool/int mutator result. Ordinary by-value parameters definitely replaced with fresh local collections stay quiet only while the replacement remains local; field/property storage, later returns, direct local aliases, intervening conditional/nested/`ref`/`out` writes, and caller-visible `ref`/`out` parameter replacements preserve the diagnostic. Local collections remain quiet because they do not outlive the scope.
...
- **DI008 keyed descriptor Type keys**: keyed `ServiceDescriptor` transient diagnostics now bind descriptor arguments by Roslyn parameter names, so a `typeof(...)` service key is not mistaken for the implementation type.
...
- **DI007 factory boundaries**: service-locator diagnostics now stay quiet inside `ServiceDescriptor` provider factories, including descriptors passed through `TryAddEnumerable`, and inside composition-root resolution from `static Main` or top-level statements.
...
- **DI003 fixer renamed user methods**: the lifetime rewrite's fix-target walk matched any ancestor invocation whose method name merely contained "Singleton"/"Scoped"/"Transient", so a user helper like `Mappers.MapScopedValue(...)` wrapping the diagnosed resolution was renamed instead of the registration — non-compiling output. Rewrite targets now require the bound method to live on the framework's own registration types (`ServiceCollectionServiceExtensions`, `ServiceCollectionDescriptorExtensions`, `ServiceCollectionHostedServiceExtensions`, or `ServiceDescriptor` in the MEDI namespaces) — a namespace-only check would still have matched user helpers declared inside `Microsoft.Extensions.DependencyInjection` for extension discoverability.
- **DI003 own-scope factory false positive**: a factory that creates and provably disposes its own scope (`using var scope = sp.CreateScope(); var seed = scope.ServiceProvider.GetRequiredService<…>();`) performs one-time scoped setup — the resolved instance does not outlive the factory call, so nothing is captive. Resolution receivers are now classified (framework `CreateScope`/`CreateAsyncScope` surface only — user overloads declared in the MEDI namespace do not qualify; any dispose before the last use of the resolved value kills the proof even for `using var` scopes; only same-execution-context resolutions qualify — a nested lambda resolving from the outer factory's scope runs after disposal and still reports; an explicit `Dispose()` only proves cleanup when it dominates the resolution — conditional, nested-lambda, or fire-and-forget `DisposeAsync()` calls do not) and the suppression additionally requires the resolved instance not to escape into the product — provably-safe derived values (primitives, enums, and strings, like `seed.Config` — arbitrary structs may carry the instance in a field and do not qualify) may flow onward — a reference-typed member result such as `seed.GetSelf()` may carry the instance and still reports — while the instance itself (`return new Singleton(seed)`), a method group capturing it (`seed.DoWork`), or a closure over it in a nested lambda (`() => seed.Config`) still reports; the explicit dispose must also run after the last use of the resolved value with no return/throw/jump in between, so consuming the service from an already-disposed scope — or leaking it through an early exit — is never blessed, while the `try { … return …; } finally { scope.Dispose(); }` idiom is honored because finally always runs. An undisposed factory scope also stays reported because the scope and its scoped instances live as long as the product.
