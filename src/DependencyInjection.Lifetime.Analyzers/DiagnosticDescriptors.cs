...
public static class DiagnosticDescriptors
{
    ...
    public static readonly DiagnosticDescriptor RxSubscriptionLeak = new(
        ...
        defaultSeverity: DiagnosticSeverity.Warning,
        isEnabledByDefault: true,
        description: "IObservable<T>.Subscribe returns an IDisposable that unsubscribes the observer when disposed. A transient or scoped service that subscribes to an observable exposed by a longer-lived publisher (a singleton dependency, a scoped publisher shared by a transient, or a direct static readonly observable field with a stable process-lifetime identity) and discards that token — as an ignored expression statement, a discard assignment, a never-referenced local, or an otherwise-unused private field — leaves the observer registered for the publisher's whole lifetime. The publisher roots every subscriber instance the container ever creates, leaking memory on each resolution and invoking stale observers against released state. Store the token and dispose it when the subscriber is released (for example in Dispose, or via a CompositeDisposable).",
        customTags: WellKnownDiagnosticTags.CompilationEnd
    );
