...
**Guardrails:** the scope must be disposed by a `using` in the same method — an undisposed scope has no proven teardown point here and is DI001's finding instead. A task that is awaited, returned, stored in a local, or waited on synchronously (`.Wait()`, `.GetAwaiter().GetResult()`) keeps the frame alive and stays silent, as does background work that captures nothing scope-derived.
