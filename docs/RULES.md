...
**Guardrails:** only the selector of a `Select`/`SelectMany`, or a lambda handed to `WhenAll` directly, counts as a concurrent body — a `Where` predicate runs one element at a time during enumeration, and a lambda nested inside a selector is part of that selector's single task. Properties are excluded, since a computed property can return a fresh instance per access. Only values declared *outside* the concurrent body count — a context created or resolved inside the lambda belongs to that one task. Thread-safe services are untouched, `nameof` is not a use, and a sequential `foreach` never fans out. Known false negative: a `SelectMany` selector that returns its own inner projection fans out from that inner selector, which this rule reads as part of the outer task. `Parallel.For`/`ForEach`/`ForEachAsync` bodies and framework message handlers are DI021's territory; this rule covers the `Task.WhenAll` leg it documented as out of scope.
